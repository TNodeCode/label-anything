...
def to_coco(images_meta, annotations, categories):
    return {
        "images": images_meta,
...
st.markdown(
    "Upload images, type comma-separated labels, and export COCO annotations."
)
...
model_id = st.text_input(
    "Grounded DINO model id", "IDEA-Research/grounding-dino-tiny"
)
...
if run_button:
    ...
    if not user_labels:
        ...
        for i, uploaded_file in enumerate(images_list):
            progress.progress((i + 1) / n_images)
            ...
            for box, score, lbl in zip(boxes, scores, labels_out):
                ...
                if isinstance(lbl, str):
                    ...
                    cat_id = match_label(str(lbl), user_labels)
            ...
            if len(bboxes_xyxy) > 0:
                det = Detection.from_bboxes(
                    np.array(bboxes_xyxy).astype(int),
                    class_id=np.array(class_ids),
                )
            else:
                det = Detection.from_bboxes(np.zeros((0, 4)).astype(int))

            input_image_view.image(img, caption=uploaded_file.name)

            annotated = Detection.plot_detections(
                img.copy(), det, show_masks=False, show_bboxes=True
            )
            output_image_view.image(annotated, caption="Detections")

            image_counter += 1

        coco_dict = to_coco(images_meta, annotations, categories)
        ...
        st.download_button(
            ...
            coco_json,
            file_name="grounded_dino_annotations.json",
