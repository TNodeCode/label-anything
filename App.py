...
CSV_FILES_DIR = "."
TRAIN_IMAGE_FILES_DIR = os.path.expanduser(
    "~/PycharmProjects/DeepLearningProjects/PersonalProjects/CVDataInspector/datasets/spine/train"
)
TRAIN_ANNOTATION_FILE = "df_merged_union_train.csv"
VAL_IMAGE_FILES_DIR = os.path.expanduser(
    "~/PycharmProjects/DeepLearningProjects/PersonalProjects/CVDataInspector/datasets/spine/val"
)
VAL_ANNOTATION_FILE = "df_merged_union_valid.csv"
TEST_IMAGE_FILES_DIR = os.path.expanduser(
    "~/PycharmProjects/DeepLearningProjects/PersonalProjects/CVDataInspector/datasets/spine/test"
)
...
def get_model(model_name: str):
    if model_name.startswith("sam_") or model_name.startswith("sam2"):
...
def predict_masks_by_bboxes(
    model_name: str, images_path: str, annotations: AnnotationSet
) -> list[dict]:
    ...
    for i, annotation in enumerate(annotations):
        ...
        for j, bbox, cls in enumerate(bboxes):
            ...
            mask_image = Image.fromarray(
                result[0].masks.data.detach().cpu().squeeze().numpy()
            )
            # save mask as PNG image
            output_image_path = os.path.expanduser(
                f"{images_path}-seg-l/{filename}-mask{j}-cls{cls}.png"
            )
...
st.markdown(
    "This app allows you to use the Segment Anything model to create segmentation masks for your datasets that have already been labelled with bounding boxes."
)
...
if annotation_format == "COCO" and annotation_source.strip() and btn_execute:
    annotations = list(
        AnnotationSet.from_coco(file_path=os.path.expanduser(annotation_source))
    )
    ...
    for i, annotation in enumerate(annotations):
        ...
        for j, bbox in enumerate(annotation.boxes):
            ...
            # result of runnign image through segmentation model
            result = sam_model(
                input_image_path, bboxes=[bbox.xmin, bbox.ymin, bbox.xmax, bbox.ymax]
            )

            # extract mask as 2D boolean numpy array from result
            mask = (
                result[0].masks.data.detach().cpu().squeeze().numpy().astype(np.uint8)
            )
            # convert 2D numpy boolean array to image object
            mask_image = Image.fromarray(
                result[0].masks.data.detach().cpu().squeeze().numpy().astype(np.uint8)
            ...
            # save mask as PNG image
            output_image_path = os.path.expanduser(
                f"{output_dir}/{os.path.basename(annotation.image_id)}-mask-{j}-cls-{bbox.label}.png"
            )
            mask_image.save(output_image_path)

            detection_mask = Detection.from_masks(np.array([mask]))

            input_image_view.image(
                Detection.plot_detections(input_image.copy(), detection_box)
            )
            output_image_view.image(
                Detection.plot_detections(
                    input_image.copy(), detection_mask, show_bboxes=False
                )
            )
